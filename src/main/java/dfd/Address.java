...
import dfd.errors.MalformedAddressException;
...
public class Address extends ByteSerializable {
    ...
    public static final String ADDRESS_PREFIX = "D";
    public final static byte default_version = 30;

    private Byte[] addr;
    public static final Integer addrSize = 25;
    ...
    private byte[] calculateChecksum(byte[] data) {
        byte[] checksum = new byte[256 / 8];
        SHA256Digest sha256Digest = new SHA256Digest();
        sha256Digest.update(data, 0, data.length);
        sha256Digest.doFinal(checksum, 0);
        sha256Digest.reset();
        sha256Digest.update(checksum, 0, checksum.length);
        sha256Digest.doFinal(checksum, 0);
